...
export default function MainContent({
  ...
  return (
    <div className="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6 relative z-10">
      {/* Search Bar */}
      <div ref={searchRef} className="bg-[#fffaf2] border-[2px] sm:border-[3px] border-[#473025] rounded-[10px] sm:rounded-[12px] h-[44px] sm:h-[51px] px-3 sm:px-4 flex items-center gap-2 sm:gap-3 mb-4 sm:mb-6">
        ...
        <input
          ...
          className="flex-1 bg-transparent font-quicksand font-bold text-[#473025] text-[13px] sm:text-[15px] placeholder:text-[#bfa183] focus:outline-none min-w-0"
        />
      </div>

      {/* Filter Row */}
      <div ref={filtersRef} className="flex flex-col sm:flex-row sm:flex-wrap items-stretch sm:items-center gap-3 sm:gap-4 mb-4 sm:mb-6">
        {/* Subject Filter */}
        <div className="relative flex-1 sm:flex-initial min-w-0">
          <div className="bg-[#f8ecdd] border-[2px] sm:border-[3px] border-[#c8a787] rounded-[10px] sm:rounded-[12px] h-[44px] sm:h-[51px] px-3 sm:px-4 flex items-center gap-2 sm:min-w-[200px]">
            <div className="flex-shrink-0">
              <BookIcon />
            </div>
            <select
              ...
              className="flex-1 bg-transparent font-quicksand font-bold text-[#b88350] text-[13px] sm:text-[15px] focus:outline-none appearance-none cursor-pointer min-w-0"
            >
      ...
      {/* Results Count */}
      {!loading && !error && (
        <p className="font-quicksand font-bold text-[#bfa183] text-[13px] sm:text-[15px] mb-3 sm:mb-4">
          Showing <span className="text-[#473025]">{games.length}</span> {games.length === 1 ? 'game' : 'games'}
        </p>
      )}
      ...
      <div ref={cardsContainerRef}>
        {loading ? (
          <div className="flex items-center justify-center h-48 sm:h-64">
            <div className="text-[#473025] font-quicksand font-bold text-lg sm:text-xl">
              Loading games...
            </div>
          </div>
        ) : error ? (
          <div className="bg-red-100 border-[2px] sm:border-[3px] border-red-400 rounded-[12px] sm:rounded-[15px] p-4 sm:p-6 text-center">
            <p className="font-quicksand text-red-700 text-base sm:text-lg">{error}</p>
          </div>
        ) : games.length === 0 ? (
          <div className="bg-[#fff6e8] border-[2px] sm:border-[3px] border-[#473025] rounded-[12px] sm:rounded-[15px] p-8 sm:p-12 text-center">
            <div className="flex justify-center mb-4 text-[#473025]">
              <div className="w-16 h-16 sm:w-20 sm:h-20">
                <NoResultsIcon />
              </div>
            </div>
            <h3 className="font-quicksand font-bold text-[#473025] text-[20px] sm:text-[24px] mb-2">
              No Games Found
            </h3>
